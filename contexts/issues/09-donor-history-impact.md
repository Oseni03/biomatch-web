...
| Deliverable | Files |
|---|---|
| `getDonorHistory` server action (paginated) | `servers/user.ts` |
| `getLocalDemandStats` server action | `servers/user.ts` |
| `useDonorHistory`, `useLocalDemandStats` hooks | `hooks/use-donor-history.ts` |
| `/donor/history` page — impact stats, history table, local demand, pagination | `app/donor/history/page.tsx` |
| Re-eligibility banner on main donor dashboard | `app/donor/page.tsx` |

## Acceptance criteria

- [x] Donor can view full donation history with date, hospital, blood group, points
- [x] Donor can see monthly emergency counts near their location
- [x] Eligibility countdown shows days until next eligible date
- [x] When eligible, banner/notification informs donor they can donate again
- [x] Re-eligibility check runs on login and via page load
- [x] Personal impact stats (total donations, points, lives) are displayed
