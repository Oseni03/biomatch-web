...
export function StaffAccounts({ hospitalId }: StaffAccountsProps) {
	...
	const currentUserRole = staffList.find((s) => s.id === currentUserId)?.role;
	...
	const handleAddStaff = async (e: React.FormEvent) => {
		e.preventDefault();
		if (!newName || !newEmail || !currentUserId) return;
		try {
			await inviteStaffMember(hospitalId, newEmail, newName, newRole, currentUserId);
	...
	const handleRoleChange = async (userId: string, role: StaffRole) => {
		...
		try {
			await updateStaffRole(userId, role, currentUserId);
	...
	const handleRemove = async (userId: string) => {
		if (!currentUserId) return;
		try {
			await removeStaffMember(userId, currentUserId);
							...
							>
								...
								<div className="flex items-center gap-3">
									{isAdmin ? (
										<select
											value={st.role}
											disabled={!st.isActive || st.id === currentUserId}
											onChange={(e) =>
									...
									{isAdmin && st.isActive && st.id !== currentUserId && (
										<button
