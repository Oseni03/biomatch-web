...
import { Wallet, Award, HeartHandshake, Dumbbell, Lock, CheckCircle2, Ticket } from "lucide-react";
...
export default function DonorWalletPage() {
  ...
  const fetchWallet = useCallback(async () => {
    ...
    const [{ data: walletRow }, { data: claimRows }] = await Promise.all([
      supabase.from("biomatch_wallets").select("points, lifetime_donations").eq("user_id", userId).single(),
      supabase.from("biomatch_incentives_claims").select("id, type, status").eq("user_id", userId),
    ]);

    if (walletRow) setWallet(walletRow as WalletData);
    if (claimRows) setClaims(claimRows as Claim[]);
  }, []);
  ...
  const hasActiveClaim = (type: Claim["type"]) =>
    claims.some((c) => c.type === type && (c.status === "approved" || c.status === "redeemed"));
  ...
  const handleRedeem = async (type: Claim["type"]) => {
    ...
    setRedeeming(type);
    const { error } = await supabase.from("biomatch_incentives_claims").insert({
      user_id: userId,
      type,
      status: "pending",
      metadata: {},
    ...
    <div className="space-y-8">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-rose-600 to-rose-500 p-6 text-white shadow-lg">
        <div className="flex items-center gap-2 text-rose-100">
          ...
          <span className="text-sm font-medium">BioMatch Rewards Wallet</span>
        </div>
        <div className="mt-4 flex flex-wrap items-end gap-8">
          <div>
            <p className="text-xs text-rose-100">Loyalty Points</p>
            <p className="text-4xl font-bold">{loading ? "—" : points}</p>
          </div>
          <div>
            <p className="text-xs text-rose-100">Lifetime Donations</p>
            <p className="text-2xl font-semibold">{loading ? "—" : wallet?.lifetime_donations ?? 0}</p>
          </div>
        </div>
        <p className="mt-4 max-w-md text-xs text-rose-100">
          Points are earned through verified donations and redeemed for non-cash health and
          wellness perks below.
        </p>
      </div>
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5">
      <div className="flex items-start justify-between">
        ...
        {unlocked && !redeemed && <CheckCircle2 className="h-4 w-4 text-emerald-500" />}
      </div>
      ...
      >
        ...
        {redeemed ? "Voucher Redeemed" : loading ? "Processing..." : unlocked ? "Redeem Voucher Code" : "Locked"}
      </button>
    </div>
  );
}
