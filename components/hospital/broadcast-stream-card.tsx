...
export function BroadcastStreamCard({
	...
	return (
		<div
			className={`bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl p-6 shadow-sm transition-all duration-300 relative ${
				req.status === "completed" ? "opacity-75 border-gray-200" : ""
			}`}
		>
			...
			{req.status !== "completed" && (
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 dark:bg-zinc-950 border border-gray-100 dark:border-zinc-850 rounded-2xl mb-6">
					<div className="text-center md:border-r border-gray-200 dark:border-zinc-800">
						<span className="text-[22px] font-bold font-mono text-zinc-900 dark:text-white block">
							{funnel.alerted}
						</span>
					...
					<div className="text-center">
						<span className="text-xs font-mono font-bold text-red-600 block uppercase pt-2">
							{req.status === "matched"
								? "Transit Active"
								: "Searching..."}
						</span>
			...
			{req.status === "matched" && funnel.donorName && (
				<div className="p-4 bg-green-50/50 dark:bg-green-950/10 border border-green-150 rounded-2xl mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
			...
			{req.status === "completed" && (
				<div className="p-4 bg-gray-50 dark:bg-zinc-950 border border-gray-150 rounded-2xl mb-2 flex items-center gap-3">
			...
			<div className="flex justify-between items-center text-xs text-gray-400 border-t border-gray-100 dark:border-zinc-850 pt-4">
				<span>
					Pints Requested:{" "}
