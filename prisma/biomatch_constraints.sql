...
-- UPDATE-first, never INSERT..ON CONFLICT: Postgres validates CHECK
-- constraints on the *proposed* row before conflict detection, so an upsert
-- would reject every negative delta (voucher debit) even when the balance
-- covers it. A plain UPDATE evaluates the CHECK on the final balance, which
-- is exactly the overdraft protection we want.
